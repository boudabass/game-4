    ...
    const handleEnd = (e) => {
        if (InputManager.isDragging) {
            ...
            if (wasClick) {
                ...
                // Modification pour Elsass Frost v1 + Player
                // Priorité au mode Construction / Démolition
                if (window.BuildingSystem && (BuildingSystem.isPlacing || BuildingSystem.isDemolishing)) {
                    BuildingSystem.handleWorldClick(worldX, worldY);
                } else {
                    // Mode Normal : Sélection Bâtiment
