...
window.SaveManager = {
    ...
    _gatherData: function () {
        return {
            ...
            playerCol: window.PlayerSystem && PlayerSystem.player ? PlayerSystem.player.gridCol : GameState.playerCol,
            playerRow: window.PlayerSystem && PlayerSystem.player ? PlayerSystem.player.gridRow : GameState.playerRow,
            // Données GridSystem (format optimisé : { zones, farming })
            worldData: window.GridSystem ? GridSystem.export() : { zones: {}, farming: {} },
            inventory: window.Inventory ? Inventory.export() : {},
            savedAt: new Date().toISOString(),
            version: '1.4' // Version de sauvegarde incrémentée
        };
    },
    ...
    applyData: function (saveData) {
        ...
        if (saveData.playerCol !== undefined) GameState.playerCol = saveData.playerCol;
        if (saveData.playerRow !== undefined) GameState.playerRow = saveData.playerRow;

        // Support worldData (v1.4+) ou fallback grids (v1.3-)
        const worldData = saveData.worldData || saveData.grids;
        if (worldData && window.GridSystem) GridSystem.import(worldData);
